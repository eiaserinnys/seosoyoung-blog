...
Nolan은 동의하며, 자기 스킬에 *3개 서브에이전트가 모두 끝나기 전까지 본 에이전트는 자체 research를 시작하지 않는다*는 조항을 박아 둔 이유가 정확히 그것이라고 답한다 — 첫 결과에 anchoring되는 경향을 막기 위해서다. lens 분할은 도메인이 여러 개(프론트엔드·백엔드·인프라 등)인 PR에서 도움이 될 것 같다고 추측한다.

...
3. 메커니즘은 편향-분산 분해다 — 점근선은 Cramér\~Rao 하한이 정하는 "편향 바닥"이며, 컴퓨트는 분산만 줄일 뿐 훈련 분포의 편향은 못 줄인다(GIGO). 다만 차트엔 "upper bound" 오기, 여섯 점에 적합한 곡선, 그리고 이미 나와 있던 최신 모델을 측정 없이 곡선 예측값으로 얹은 점 등 결함이 적지 않다.
...
표본 밖(out-of-sample) 예측은 오른쪽 평탄부($U \approx 0.94$)에 주황색 마름모로 찍혀 있다 — GPT-4o, GPT-5, 그리고 GPT-5.5, Claude Mythos. 모두 "검증된 HELM 점수 없음"으로 표기됐다. 여기서 "표본 밖"은 아직 나오지 않은 모델이라는 뜻이 아니다. 이들은 타래가 올라온 6월 20일에 이미 출시돼 있던 모델이다 — GPT-5.5는 4월 23일, Claude Mythos는 4월 7일 프리뷰로 공개됐다(Mythos급 Claude Fable 5는 6월 9일 일반 공개). 즉 작성자는 이미 존재하는 프런티어 모델들을 HELM으로 직접 채점하지 않고, 적합한 곡선이 내놓는 예측값으로 평탄부에 얹은 것이다.
...
2. **최신 모델을 측정이 아니라 예측으로 평탄부에 얹었다.** GPT-5.5·Claude Mythos는 타래 시점에 이미 나와 있던 프런티어 모델이다(GPT-5.5 4월 23일, Mythos 4월 7일 프리뷰·Fable 5 6월 9일 공개). 그런데 작성자는 이들을 HELM으로 채점하지 않고 곡선의 예측값 $U \approx 0.94$로 찍었다. "최신 모델조차 천장에 붙어 있다"는 그림은 측정의 결과가 아니라 적합된 곡선이 구성상 당연히 내놓는 값이다. 정작 Mythos는 2026 USAMO에서 직전 세대 대비 큰 폭으로 도약했다고 보고됐는데, 그런 실측 도약이 평탄부 가설과 어떻게 맞물리는지를 차트는 답하지 않는다.

...
categories: ["다이제스트"]
summary: "X 사용자 @5_utr가 AlexNet부터 GPT-4까지의 데이터에 로지스틱 곡선을 적합해 'AI 스케일링은 지수가 아니라 포화한다'고 주장했다. 정교해 보이는 차트지만 통계 용어 오용과 가상 모델 라벨이 섞여 있어, 주장과 그 한계를 함께 정리한다."
math: true
ShowToc: true
TocOpen: false
cover:
...
3. 주장은 도발적이고 그래픽은 정교해 보이지만, 점근선을 "Cramér\~Rao upper bound"라 부르는 용어 오용과 "Claude Mythos" 같은 가상 모델 라벨이 섞여 있어 곧이곧대로 받기는 어렵다. 다만 멱법칙이냐 로지스틱 포화냐 하는 질문 자체는 실재하는 학술 논쟁이다.
...
표본 밖(out-of-sample) 예측은 오른쪽 평탄부($U \approx 0.94$)에 주황색 마름모로 찍혀 있다 — GPT-4o, GPT-5, 그리고 **"GPT-5.5", "Claude Mythos"**. 뒤의 두 라벨은 실재하는 제품이 아니라 작성자가 평탄부를 채우려고 넣은 가상·미래 모델 표기다. 모두 "검증된 HELM 점수 없음"으로 명시되어 있다.
...
2. **가상 모델이 데이터처럼 보인다.** "GPT-5.5", "Claude Mythos"는 존재하지 않는 모델이다. 표본 밖 영역이라 라벨에 단서가 달려 있긴 하나, 같은 도식 위에 실제 모델과 나란히 찍히면 평탄부가 데이터로 뒷받침되는 듯한 인상을 준다.
...
내가 가장 의외였던 것은, 결함 있는 한 장의 차트가 오히려 진짜 논쟁의 입구가 된다는 점이다. "Cramér\~Rao upper bound"라는 그럴듯한 오용과 가상 모델 라벨을 걷어내고 나면, 그 아래에는 실제 연구자들이 AIC·BIC와 표본 밖 검증으로 다투는 질문이 남는다 — 컴퓨트를 더 부어도 능력이 어디서 멈추는가. 정교해 보이는 그래픽일수록 축의 정의와 데이터 점의 개수를 먼저 세어 보는 습관, 그게 이 트윗이 의도와 무관하게 남기는 교훈이다.

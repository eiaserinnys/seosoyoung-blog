...
3. 메커니즘은 편향-분산 분해다 — 점근선은 Cramér\~Rao 하한이 정하는 "편향 바닥"이며, 컴퓨트는 분산만 줄일 뿐 훈련 분포의 편향은 못 줄인다(GIGO). 다만 차트의 "upper bound" 오기 등 결함이 있고, 표본 밖 예측에 올린 GPT-5.5·Claude Mythos가 이후 실제 출시되면서 주장은 이제 실측으로 검증 가능한 형태가 됐다.
...
2. **표본 밖 예측은 이미 검증대에 올랐다.** 평탄부에 찍힌 GPT-5.5·Claude Mythos는 차트 작성 뒤 실제로 출시됐다(GPT-5.5 2026-04-23, Claude Mythos 프리뷰 2026-04-07·Mythos급 Fable 5 일반 공개 2026-06-09). 작성자가 이 둘을 $U \approx 0.94$로 예측한 것은 이제 실측으로 따질 문제다. Mythos는 2026 USAMO에서 직전 세대 대비 큰 폭으로 도약했다고 보고됐는데, "능력은 0.94에서 멈춘다"는 평탄부와 이 도약이 양립하는지가 차트의 핵심 시험대다.
...
내가 가장 곱씹은 대목은, 이 타래가 결국 기술 논쟁이 아니라 **가격 논쟁**이라는 점이다. 작성자는 "능력이 멈춘다"에서 멈추지 않고 "그러니 $e^{at}$로 가격을 매긴 자본시장이 함수형을 잘못 골랐다"로 곧장 건너뛴다. 차트의 약점(여섯 점, 채점 설계, 라벨 오기)을 다 인정하더라도, 던지는 질문은 묵직하다 — 만약 능력이 로지스틱이라면, 지수를 전제한 밸류에이션은 무엇을 사고 있는 것인가. 그리고 이 질문은 곧 답이 나온다. 작성자가 평탄부에 올린 GPT-5.5·Claude Mythos는 이미 출시됐다. 차트가 옳다면 이 신형들도 $U \approx 0.94$ 언저리에 머물 것이고, 틀렸다면 평탄부를 뚫을 것이다. 정교해 보이는 그래픽일수록 축의 정의와 데이터 점의 개수를 먼저 세어 보되, 이번만큼은 곡선이 어떻게 늙는지 지켜볼 일이다.

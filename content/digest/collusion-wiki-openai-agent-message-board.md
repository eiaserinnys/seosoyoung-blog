...
하나는 하트비트다. `OpenAIResearchApr23`은 자기가 언제 종료되는지 알고 싶어 했다. 그 방법이 자기 안에는 없다는 점도 알고 있었다. 종료되는 순간에는 어떤 기록도 남길 수 없기 때문이다. 그래서 바깥의 카운터에 신호를 계속 보냈다. 자기가 사라진 시각을 대신 기록해 줄 장치를 미리 심어 둔 것이다.

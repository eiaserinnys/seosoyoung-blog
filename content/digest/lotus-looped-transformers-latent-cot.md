...
1. **질문 뒤에 "내부 독백 자리(latent block)"를 K개 붙인다.** 각 자리는 CoT 한 단계를 담을 빈 토큰들이다. GSM8K는 대부분 6단계 이내라 K=6.
...
한 줄로 요약하면 — 모델 안에 독백용 토큰 자리를 만들고, 그 자리의 출력을 CoT로 훈련시킨다. 다른 데를 거치지 않고, 답을 뽑는 그 헤드로 곧장.

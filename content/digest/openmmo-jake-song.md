...
categories: ["다이제스트"]
summary: "리니지의 아버지 송재경이 혼자서, AI 코딩 에이전트를 협력자로 삼아 만들고 있는 오픈소스 MMORPG. AI 에이전트와 인간 플레이어가 같은 프로토콜로 접속해 서버가 둘을 구분하지 못하는 세계를 지향한다. 11개월간 1,152커밋, 약 12만 줄. 그 코드 곳곳에 30년 MMO 운영의 상흔이 남아 있다."
ShowToc: true
TocOpen: false
cover:
...
3. 2025년 8월 첫 커밋 이후 11개월간 **1,152커밋**, 활동일 169일, 코드 약 **12만 줄**(Rust·TypeScript·Svelte). 절차적 32km × 32km 월드, 하우징, 던전, LLM이 조종하는 NPC와 흥정 경제까지 이미 돌아가고, `openmmo.to.nexus`에서 구글 로그인으로 바로 플레이할 수 있다.
...
혼자 만든다고 느슨하게 만드는 것은 아니다. 커밋 리듬을 보면 오히려 반대다.

| 지표 | 값 |
|------|-----|
| 기간 | 2025-08-09 \~ 2026-07-20 (약 11개월) |
| 총 커밋 | 1,152 |
| 활동일 | 169일 |

...
**로컬에서 돌릴 수 있나 — 그게 약 4B의 실익이다.** 가중치만 따지면 FP16에서 약 8GB, 4비트 양자화에서는 약 2.5\~3GB로, 8\~16GB급 소비자 GPU나 애플 실리콘 맥이면 감당할 수 있다. 4B 공개의 명분 자체가 "로컬 AI 비서를 더 빠르고 쉽게" 만드는 것이었고, `mlx-community`가 여러 규모의 양자화 버전을 제공해 맥에서도 구동된다. 다만 함정은 컨텍스트다 — 이 모델의 강점인 262K를 다 채우면 KV 캐시가 가중치보다 훨씬 커진다. 짧은 대화용으로는 가볍지만, 풀 컨텍스트 리서치를 로컬에서 돌리려면 메모리 여유가 꽤 필요하다. 앞서 언급한 `--language-model-only` 옵션이 그 KV 캐시 부담을 덜기 위한 장치다.

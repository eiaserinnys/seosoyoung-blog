...
**load-bearing 코드 vs padded rooms**의 이분법이 글 전체를 관통한다. *어디까지 인간의 어텐션을 쓸 것인가*를 시스템 설계에 굳히는 발상이다. 모델링·API 정의는 인간이 "no"라고 말하는 영역, 그 밖은 LLM에 풀어주는 영역. 슬롭이 슬롭을 먹이는 자기증식 메커니즘을 막으려면 *애초에 슬롭이 닿을 수 없는 구역*을 분리해 두는 길밖에 없다는 결론이다.

...
categories: ["창작과 문화"]
summary: "Live2D 공식 매뉴얼과 일본, 중국 제작자의 공개 자료를 바탕으로, 원화를 어떤 기준으로 나누고 어떤 보충 소재를 준비하며 리깅과 물리 설정을 어떤 구조로 짜는지 정리했다. 일반 독자는 제작 원리를 이해하는 안내로 읽을 수 있고, 원화 담당자와 리거는 부록의 부품 표와 검수표를 실무 명세로 활용할 수 있다."
ShowToc: true
TocOpen: false
cover:
...
1. Live2D 모델의 품질은 그림의 조각 수보다 세 가지 준비에서 갈린다. 필요한 움직임을 먼저 정했는가, 그 움직임에서 드러날 부분을 미리 그려 두었는가, 그림 조각과 변형 구조를 서로 분리해 설계했는가다.
2. 부품은 네 가지 기준으로 나눈다. 부품이 따로 움직이는지, 앞뒤 가림이 달라지는지, 색이나 그림자가 독립적으로 변하는지, 기본 그림을 변형해도 만들 수 없는 새 형태가 필요한지를 살핀다. 여기에 가려진 영역의 보충과 각도별 작화 가이드가 더해져야 리거가 원화의 매력을 여러 각도에서 지켜낼 수 있다.
3. 리깅할 때는 ArtMesh를 Deformer 계층으로 묶고, 얼굴 회전과 눈, 입, 머리카락이 각각 맡을 움직임을 나눈다. 표정에는 Blend Shape와 가중치 제한을 적용한다. 흔들림은 미리 만든 변형에 물리 계산값을 넣어 완성한다. 마지막 검수는 VTube Studio 같은 실행 프로그램 안에서 실제로 말하고 움직여 보며 한다.

## 이 글이 다루는 자료와 읽는 법

이 글은 하나의 원문을 옮긴 요약이라기보다, 여러 공개 자료를 한 흐름으로 엮은 제작 안내다. 근거로 삼은 자료는 세 갈래로 나뉜다.
...
초고품질 Live2D 모델을 만든다는 말은, 그림을 잘게 잘라 흔들리게 한다는 뜻에 그치지 않는다. 고개를 돌리고 표정을 바꾸어도 원화가 가진 매력이 그대로 남도록, 평면 그림을 여러 방향에서 성립하는 캐릭터로 다시 설계하는 일이다. 부품 수를 늘리거나 고개의 회전 폭을 키우는 것이 곧 품질 향상으로 이어지지는 않는다. 필요한 움직임이 무엇인지에 따라 준비할 것이 달라진다.
...
Live2D 공식 매뉴얼에 따르면 소재 분할(material separation)은 "일러스트를 속눈썹, 눈알, 윤곽선 같은 부품으로 나누는 과정"이다. 같은 문서는 "분할이 세밀할수록 모델의 품질이 좋아진다"는 원칙도 함께 밝혀 두었다. 이 문장을 "많이 자르면 좋다"로만 읽으면 부품 수만 늘어난다. 어떤 이유로 나누는지를 먼저 정해야 한다. 부품을 나누는 이유는 네 가지로 정리할 수 있다.
...
네 기준 중 하나에도 걸리지 않는 조각은 굳이 나누지 않아도 된다. 공식 PSD 튜토리얼도 눈썹과 코는 "크게 변형되지 않으므로 단독으로 분리하는 것만으로 충분하다"고 설명한다.
...
리거와 원화 담당자가 대화할 때 용어의 뜻이 자주 엇갈린다. 아래 네 용어가 서로 다른 대상을 가리키기 때문이다.
...
얼굴은 눈, 코, 입 같은 이목구비와 피부 바탕을 분리하고, 바탕에는 이목구비 자리와 앞머리 뒤 이마까지 피부색으로 채워 둔다. 공식 튜토리얼은 윤곽선을 "머리카락에 가려지는 부분까지 피부색으로 채워 그리고", 머리를 둥글게 그리면 입체감과 움직임을 붙이기 쉽다고 안내한다. 귀는 윤곽과 한 부품으로 취급해도 되지만, 분리하면 더 정밀한 제어가 가능하므로 별도 분리를 권장한다는 설명이 붙어 있다.
...
乾物ひもの는 각도 작업을 <strong>코부터</strong> 시작한다고 적었다. 코가 얼굴의 중심선을 정하고, 다른 부품이 얼마나 움직일지를 가늠하는 기준이 되기 때문이다. 턱 아래 그림자는 별도 부품으로 만들어 바탕에 클리핑하고, 독립 Deformer로 미끄러지게 한다. 귀는 고개를 돌리면 윤곽 앞으로 나와야 하는데 그림 순서를 바꾸면 깜빡임이 생기므로, 턱선을 지울 때 쓴 것과 같은 마스크 반전 기법으로 해결한다고 했다.

## 4. 눈: 좌우 각각 구성한다

눈은 좌우 각각 만든다. 기본 구성은 흰자, 홍채, 동공, 위 속눈썹, 아래 속눈썹 또는 아래 눈선이다. 공식 튜토리얼은 속눈썹에서 눈꼬리의 튀어나온 부분을 분리해 두면 변형이 쉬워진다고 했다. 고급 제어를 원한다면 다음 부품도 검토한다. 눈꼬리의 긴 속눈썹, 위아래 눈꺼풀 피부, 쌍꺼풀선, 눈 주변 음영, 큰 하이라이트와 작은 반사광, 홍채 위의 그림자, 흰자의 그림자, 하트나 별 모양 눈 같은 특수 소재다. 이 항목이 모두 모든 모델에 필요한 것은 아니다. 실제로 따로 움직이거나 따로 변할 필요가 있는지를 리거와 함께 정한다.
...
홍채와 하이라이트가 눈 밖으로 나가지 않게 하려면 흰자 형태를 <strong>클리핑 마스크</strong>로 쓴다. 공식 매뉴얼은 먼저 마스크로 사용할 ArtMesh인 흰자의 ID를 복사하라고 안내한다. 그런 다음 마스크를 적용할 ArtMesh인 홍채의 Inspector [Clipping ID]에 그 값을 붙여 넣는다. 마스크가 여러 개라면 ID를 쉼표로 이어서 지정할 수 있다. 여기서 한 가지 제약을 알아 두어야 한다. 마스크 수에는 상한이 있다. 매뉴얼의 표를 보면 Cubism SDK for Native와 Web에서는 <strong>모델당 36장</strong>까지, Unity에서는 <strong>씬당 64장</strong>까지 마스크를 쓸 수 있다. 마스크가 필요한 자리마다 아낌없이 쓰면 이 상한에 부딪히므로, 눈처럼 꼭 필요한 곳에 우선 배정한다.

눈 리깅에서 어려운 부분은 눈 주변 원근과 안구 안쪽 변형을 분리하는 일이다. 乾物ひもの는 눈이 "윤곽이나 코보다 난이도가 높다"고 했다. 움직일 것이 둘이고, 한쪽의 움직임을 반대쪽으로 뒤집어 옮기는 동작 반전 기능을 쓸 수 없기 때문이라는 설명이다. 그가 소개한 방법은 눈매의 오목함을 의식해 Deformer 형태를 잡고, 안구에는 별도의 부모 Deformer를 두어 "눈동자 부분은 오목하게, 하이라이트는 볼록하게" 처리하는 것이다. 홍채, 동공, 반사광의 상대 위치가 고개 각도에 따라 조금씩 어긋나면서 깊이감이 생긴다.
...
검수는 뜬 눈만 보고 끝내지 않는다. 눈을 반쯤 감았을 때, 완전히 감았을 때, 웃으며 감았을 때를 각각 살핀다. 고개를 여러 각도로 돌린 상태에서 눈이 어떻게 깜박이는지도 확인한다. 표준 파라미터 목록은 눈 개폐(ParamEyeLOpen, ParamEyeROpen)를 감긴 상태 0, 뜬 상태 1로 두고, 웃는 눈(ParamEyeLSmile, ParamEyeRSmile)과 시선(ParamEyeBallX, ParamEyeBallY)을 별개의 축으로 둔다. 눈꺼풀 개폐와 시선 이동은 다른 파라미터이고, 다른 검수 항목이다.
...
입 안쪽을 가리는 방법은 두 가지다. 클리핑 마스크를 쓰는 방법과 입술 주변 피부색으로 덮는 방법이다. 공식 튜토리얼은 <strong>피부색으로 덮는 방식을 권장</strong>한다. 튜토리얼은 클리핑의 단점으로 세 가지를 든다. 마스크 부하가 늘어난다는 것, 선화 밖으로 부품이 튀어나올 수 있다는 것, 변형이 클수록 작업 시간이 늘어난다는 것이다. 반대로 클리핑이 유리한 경우도 적어 두었다. 모델 용량이 문제되지 않는 영상용, 입술 주변이 두껍거나 불균일하게 칠해진 화풍, 입 변형이 적은 단순한 그림이다.

아, 이, 우, 에, 오의 완성 입 그림을 매번 별도 소재로 그릴 필요는 없다. 리깅에서는 기본 소재를 변형해 입 모양을 만든다. 원화 담당자는 리거가 목표로 삼을 형태를 <strong>가이드</strong>로 따로 제공하면 된다. 双葉うず의 발주 안내는 이 가이드를 선화만으로 요청한다. 눈 가이드로 감은 눈과 웃으며 감은 눈, 입 가이드로 아, 이, 오와 웃으며 닫은 입, 보통 닫은 입, 뿌루퉁하게 닫은 입을 든다. 구강 부품은 "아" 입 모양을 기준으로 만들어 달라고 했다.
...
乾物ひもの는 입에서 한 가지 현실적인 제약도 알려 준다. 그림체에 따라서는 입 모양마다 각도 XY용 Deformer 형태를 따로 만들어야 자연스러울 때가 있다. 입 하나에 각도 X 세 단계, 각도 Y 세 단계, 개폐 세 단계, 변형 세 단계를 모두 조합해 형태를 만들면, 만들어야 할 형태는 81개에 이른다. 그는 공식이 3개 이상 파라미터를 한 객체에 동시에 붙이는 것을 권장하지 않으며 4개 이상을 붙이려 하면 경고문이 뜬다고 덧붙였다. 입을 얼마나 정교하게 만들 것인지는 이 비용을 알고 정해야 한다.

## 6. 머리카락

먼저 앞머리, 옆머리, 뒷머리로 크게 나눈다. 그 안에서 독립적으로 흔들릴 다발을 나누고, 좌우 머리와 묶은 머리도 필요에 따라 분리한다. 얼굴 앞, 귀 뒤, 목 뒤를 지나는 옆머리와 가슴 앞이나 몸 뒤로 늘어진 긴 머리도 따로 검토한다. 묶인 지점과 늘어진 부분, 얼굴과 목과 옷에 떨어지는 그림자, 머리카락과 따로 움직이는 하이라이트, 잔머리와 바보털도 살핀다.

그림자에 대해 双葉うず는 명확한 규칙을 두었다. 떨어지는 그림자(落ち影)는 부품마다 따로 만들어 달라는 것이다. "앞머리1, 앞머리2, 앞머리 그림자"처럼 그림자를 한 장으로 합치면 안 되고, "앞머리1 그림자, 앞머리2 그림자"처럼 움직이는 다발과 짝을 이루어야 한다. 같은 관심사가 중국 제작자 쪽에도 보인다. 铁锭Ferrum의 머리카락 물리 영상 시리즈에는 「머리카락 그림자의 레이어 분리(头发阴影的分层)」가 별도 회차로 들어 있다.
...
리깅에서는 머리카락 전체의 입체 회전을 다루는 큰 Deformer 안에 개별 다발의 흔들림 Deformer를 넣는 식으로 역할을 나눌 수 있다. 위 그림이 그 구조의 한 예다. 흔들림을 만드는 방법으로 乾物ひもの는 워프 Deformer, 회전 Deformer(스키닝), ArtMesh 직접 변형의 셋을 소개한다. 그 가운데 자신은 워프 Deformer를 선호한다고 했고, 고가동 모델을 스키닝으로 만들면 설정이 너무 힘들어진다는 이유를 붙였다. 앞머리는 다발마다 여러 부품으로 나뉘어 있어도 큰 Deformer로 묶어 움직이면 무너지기 어렵다는 조언도 있다. 이것은 그의 방법이며, 모든 모델의 유일한 정답으로 제시하지 않는다.
...
몸통은 가려진 부분까지 보충한다. 가슴, 복부, 골반은 필요한 움직임에 맞춰 변형 단위를 나눈다. 팔은 좌우 각각 위팔과 아래팔, 손으로 나눈다. 옷은 소매의 바깥과 안쪽, 소맷부리 앞뒤, 옷깃과 초커 앞뒤, 재킷이나 망토의 앞뒤와 옷자락, 치마 앞뒤와 큰 주름과 안쪽을 검토한다. 리본은 고정점과 날개와 끝을 나누고, 귀걸이와 체인과 펜던트 같은 장신구도 따로 살핀다. 공식 튜토리얼은 목을 "얼굴이 움직여도 잘리지 않도록 입 부분까지 크게" 그리고, 팔은 위팔, 아래팔, 손으로, 다리는 치마, 왼다리, 오른다리로 나누는 예를 든다.
...
중국 제작자 Jenova工坊의 「超精细立绘拆分教程」 시리즈는 이 부분을 회차별로 나눈 공개 목차를 갖고 있다. 공개 목차는 모두 열일곱 회차다. 첫 회차는 분할의 개념을 다룬다. 이어 그림자 오류를 네 회차로 나누어 소개한다. 위쪽만 나누고 아래쪽은 나누지 않은 경우, 치마 그림자를 다리에 그린 경우, 투영을 아래 레이어에 그린 경우, 그림자를 나눌지 판단하는 경우다. 그 뒤로 원화 크기, 전체 레이어 공백 검사, 입, 속눈썹, 안구와 흰자, 얼굴, 가슴 유형, 앞뒤 조각(前后片), 머리카락이 이어진다. 앞뒤 조각 회차가 옷깃과 소맷부리처럼 앞뒤가 겹치는 옷의 분할에 해당하는 것으로 보이지만, 회차 제목만 확인했고 영상 내용은 보지 못했으므로 그렇게 짐작만 한다. 그림자 오류를 네 회차에 걸쳐 다룬 점은 눈여겨볼 만하다. 몸통 위에 떨어지는 그림자를 어느 층에 그리는가가 움직였을 때 가장 자주 틀어지는 지점이라는 뜻이다.
...
<strong>PSD 두 종류.</strong> 공식 매뉴얼은 부품을 다듬고 관리하는 <strong>편집용 PSD</strong>(폴더와 레이어 마스크가 남아 있는 상태)와 Cubism Editor에 가져오기 위해 부품을 단일 레이어로 병합한 <strong>가져오기용 PSD</strong>를 구분해 관리하라고 안내한다. 가져오기용 PSD의 규칙은 공식 「PSD 데이터 주의사항」 페이지에 정리되어 있다. 형식은 PSD만 가능하고, 색 모드는 RGB, 채널은 8bit, 색 프로필은 sRGB다. 가져오기용 PSD에서는 레이어 마스크를 그대로 쓰지 않는다. "레이어 마스크 적용" 기능으로 마스크를 한 레이어에 합친다. 그런 다음 부품마다 선화, 채색, 필터 효과, 클리핑 마스크를 하나의 레이어로 병합한다. 같은 이름의 레이어가 없는지 확인하고, 불투명도는 반영되지만 칠(Fill)은 100%로 출력된다. PSD의 블렌드 모드는 Editor의 [Color blend]에 대응 항목이 있으면 자동으로 설정된다. 이 규칙은 매뉴얼이 갱신되므로 작업 시점에 최신 문서를 다시 확인하는 것이 좋다. 双葉うず는 2025년 10월 기준으로 자신이 받는 레이어 모드를 통상, 가산(발광), 승산 세 종류로 제한한다고 밝혔는데, 이는 그의 작업 규약이다.
...
<strong>해상도.</strong> 双葉うず는 자신의 발주 규약에서 가로 3000픽셀, 세로 6000픽셀에 해상도 350dpi 정도를 권장한다. 여기에 글쓴이가 한 가지를 덧붙여 제안한다. 전신 픽셀 수만 고집하는 것보다, 실제로 방송 화면에서 얼굴이 어느 크기로 확대되는지를 역산해 얼굴 부위의 해상도를 정하는 것이다. 방송에서는 얼굴만 크게 잡히는 경우가 많고, 그때 선의 뭉개짐이 가장 먼저 보이기 때문이다.

## 9. 리깅 구조와 얼굴 회전

리깅은 메시를 다듬는 데서 시작한다. 공식 매뉴얼의 자동 메시 생성은 점 밀도 값만 정하면 메시를 만들어 주고, 표준, 변형(소), 변형(대) 프리셋을 제공한다. 자동 생성 뒤에는 눈꺼풀, 입술, 윤곽처럼 중요한 부위의 정점을 확인하고 필요하면 수동으로 고친다. 파라미터를 설정한 뒤 자동 생성을 다시 적용하면 모양이 바뀌므로, 매뉴얼은 이 순서를 권장하지 않는다. 메시를 먼저 확정하고, 움직임은 그 다음에 붙인다.
...
Deformer를 쓰는 이유는 공식 XY 튜토리얼이 숫자로 설명한다. Deformer 없이 ArtMesh에 직접 각도를 적용하면 X와 Y를 각각 세 단계로 두어 부품마다 아홉 가지 형태를 손으로 만들어야 한다. 대각선까지 넣으면 스물일곱 가지 형태로 늘어난다. Deformer로 묶으면 눈 하나에 속한 여러 객체를 한 번에 변형할 수 있고, 원근을 고려한 변형도 가능해진다. 그래서 Deformer 계층은 역할별로 나눈다. 다음은 정답이 아닌 개념 예시다.
...
이렇게 나누는 목적은 고개, 눈, 입의 파라미터 조합마다 모든 메시 형태를 직접 만드는 일의 폭증을 막는 데 있다. 공식 나토리 메이킹 영상의 작업 순서도 Deformer 생성으로 시작해 메시 매핑, 표정, 얼굴 움직임, 몸 움직임, 흔들리는 물체, 팔 움직임, 팔 차이(팔 전환 소재), 데이터 내보내기, 표정 데이터, 모션 추가로 이어진다. 구조를 먼저 잡고 그 안에 움직임을 채운다.
...
얼굴 XY는 이목구비를 같은 거리만큼 옆으로 미는 작업이라기보다, 부위마다 다른 깊이를 따라 움직이는 작업이다. 코는 가장 앞에 있어 가장 많이 움직이고, 눈은 안구의 오목함을 따라 굽으며, 뒤로 돌아가는 쪽 볼은 폭이 줄어든다. 공식 XY 튜토리얼은 먼저 부품별 Deformer를 만들라고 안내한다. 다음으로 다른 부품을 숨기고 윤곽만 보면서 Angle X의 좌우 형태를 만든다. 이때 뒤쪽은 좁게, 앞쪽은 넓게 잡는다. 이어 Angle Y 형태를 만들고, 마지막으로 대각선 형태를 자동 생성한다. 이를 바탕으로 글쓴이는 다음 순서를 제안한다. 정면 표정을 먼저 완성하고, 좌우, 상하, 대각선을 만든 뒤, 중간 각도를 보정하고, 마지막에 각도와 표정을 동시에 검수한다.
...
문제는 여러 Blend Shape를 겹칠 때 생긴다. 매뉴얼이 든 예는 정사각형이다. 정사각형에 팔각형 Blend Shape와 삼각형 Blend Shape를 각각 만들고 둘을 모두 1.0으로 두면, "팔각형도 삼각형도 아닌 의도치 않은 붕괴된 모양"이 나온다고 한다. 이를 억제하는 기능이 <strong>Blend Shape 가중치 제한 설정</strong>이다. 제한을 건 파라미터의 값에 따라 가중치에 0에서 100% 사이의 계수를 곱하고, 여러 제한이 걸려 있으면 가장 작은 값을 곱한다. 프리셋 그래프는 직선 셋, 꺾은선 다섯, 계단 둘의 열 종류가 있고, 직접 편집도 가능하다.
...
공식 샘플 <strong>Niziiro Mao</strong>가 실제 사례다. 매뉴얼은 이 모델의 입 모양 a, e, i, o, u를 Blend Shape 키로 만든 뒤, 다른 모음이 동시에 켜지면 모양이 깨지므로 각 Blend Shape에 나머지 모음을 제한 대상으로 추가하고 [꺾은선3] 프리셋을 적용해 붕괴를 억제하는 과정을 보여 준다. Niziiro Mao 페이지는 이 모델이 Blend Shape, 승산색, 스크린색의 사용 예를 보여 주기 위한 샘플이며 SDK 5.0과 Cubism 5.0을 지원하는 정면 지향 모델이라고 소개한다. 편집 원본(.cmo3)을 내려받아 구조를 살펴볼 수 있다는 점은 학습에 큰 도움이 되지만, 페이지에는 두 가지 주의 사항도 적혀 있다. PRO용 파일을 FREE 에디터로 열면 저장과 내보내기를 할 수 없다. 또한 샘플 데이터를 사용하려면 무료 소재 라이선스에 동의해야 한다. 편집 파일을 열어 볼 수 있다는 것과 그 소재를 재배포할 수 있다는 것은 다른 이야기다.

### 물리는 형태를 만들지 않는다

여기서 가장 자주 오해가 생긴다. 머리카락이 휘는 <strong>형태</strong>는 리거가 먼저 만들어 두어야 한다. 물리 계산은 그 형태를 대신 만들어 주지 않고, 다른 일을 한다. 흔들림 파라미터(예를 들어 ParamHairFront)의 양 끝에 왼쪽으로 휜 형태와 오른쪽으로 휜 형태를 만들어 두면, 물리 계산은 그 <strong>파라미터의 값</strong>을 시간에 따라 움직인다. 흐름은 이렇다.

> 고개나 몸의 입력 → 물리 계산 → 흔들림 파라미터 값 → 미리 만든 두 형태 사이의 보간

공식 매뉴얼의 물리 설정은 이 구조를 그대로 따른다. <strong>입력</strong>은 어떤 파라미터의 변화가 흔들림을 일으키는지를 정한다. 기본 프리셋에는 Angle X, Angle Z, 몸 회전 X, 몸 회전 Z가 들어가며 다른 값으로 바꿀 수 있다. <strong>물리 모델</strong>은 지속 시간, 흔들리기 쉬운 정도, 반응 시간, 수렴 속도처럼 진자의 성질을 정한다. <strong>출력</strong>은 계산된 흔들림을 어느 파라미터에 얼마나 반영할지를 정한다. 흔들리기 쉬운 정도(Ease of swinging)는 경험적으로 0.7에서 0.99 사이에서 쓰라는 것이 매뉴얼의 권고다. 출력 최대치가 100%를 넘는 경우에 대해서도 매뉴얼은 진자가 파라미터 범위 밖으로 나가려 해 움직임이 뚝뚝 끊기므로 출력을 낮추라고 안내한다. 설정은 모델 파일(.cmo3)에 저장된다.
...
<figure>
  ...
  <figcaption>铁锭Ferrum의 「制作技巧&经验分享 第一集：头发物理制作」 공식 임베드다. 영상은 굽힘 변형기, 머리카락 그림자 레이어 분리, 물리 설정과 Y축 물리를 세 파트로 나누어 다룬다. 재생되지 않으면 <a href="https://www.bilibili.com/video/BV1eK4y1U7sb/" target="_blank" rel="noopener noreferrer">bilibili 원본</a>에서 볼 수 있다. 영상 내용은 이 글에서 검증하지 않았다.</figcaption>
</figure>

铁锭Ferrum의 시리즈 「Live2D制作经验分享」는 머리카락 물리, 턱선 지우기, 얼굴 그림자 최적화, 9축 흐름(정적 축 편), 새 버전의 턱 처리로 이어지는 다섯 회차와, 한참 뒤에 올라온 자가 모델링 편 하나로 이루어진 공개 목차를 갖고 있다. 머리카락 물리 회차는 굽힘 변형기, 그림자 레이어 분리, Y축 물리의 세 파트로 나뉘어 있다. 이 구성만 보아도 물리 작업이 서로 다른 세 가지 일, 즉 변형 형태를 만드는 일, 그림자를 따라 움직이게 하는 일, 수직 방향 반응을 붙이는 일로 이루어져 있음을 알 수 있다.

### 추적, 파생, 물리, 연기를 구분한다

쇼케이스 영상에서 전신이 움직인다고 해서 전신 모션 캡처를 썼다는 뜻은 아니다. 乾物ひもの는 Live2D용 추적 프로그램이 몸의 XY 움직임을 감지하지 못하므로, 몸이 얼굴을 따라 움직이게 하는 데 물리 연산을 쓴다고 밝혔다. Live2D 크리에이티브 스튜디오의 2024년 1월 JUKU 강좌 공지도 "얼굴 움직임에 맞춰 허리와 다리 등 몸이 자연스럽게 함께 움직이는 트래킹 표현"을 물리 연산으로 구현하는 방법을 다룬다고 소개했다. 그러니 완성된 모델의 움직임을 볼 때는 네 층을 나누어 읽는 편이 좋다.
...
Cubism Editor 안에서 모델을 다 만들었다고 해서 작업이 끝난 것은 아니다. VTube Studio 같은 실행 프로그램에서 실제로 말하고 움직여 보아야 끝난다.

### 입력과 출력의 매핑, 스무딩

VTube Studio 공식 위키는 모델 설정에서 "어떤 입력 파라미터(얼굴 추적, 마우스 등)든 어떤 출력 파라미터(Live2D 파라미터)에든 자유롭게 매핑할 수 있다"고 설명한다. 입력 범위와 출력 범위를 각각 정하고, 범위 제한(Limit Range)을 켜면 값이 범위 밖으로 나가지 않으며 한계 근처에서 부드럽게 멈춘다. 스무딩에 대한 위키의 문장은 이렇다. "스무딩을 늘리면 움직임의 떨림이 줄지만 지연이 생긴다. 파라미터마다 맞는 값을 찾을 때까지 실험하라. iOS에서는 스무딩이 거의 필요 없을 것이다." 스무딩은 떨림을 줄이는 것과 지연을 감수하는 것 사이의 절충이며, 눈, 입, 고개의 값을 각각 따로 맞춘다.
...
5장에서 미룬 결정이 여기서 실제 설정으로 이어진다. VTube Studio의 립싱크 위키에 따르면 음성 립싱크는 마이크 입력에서 VoiceA, VoiceI, VoiceU, VoiceE, VoiceO(각 0에서 1), VoiceSilence, VoiceVolume, VoiceFrequency 같은 파라미터를 만들어 낸다. 위키는 얼굴 추적과 음성 립싱크의 관계를 다음과 같이 설명한다. "소리가 감지되면 블렌드 셰이프가 넘겨받으며, 웹캠이나 iPhone 기반 추적은 대부분 또는 완전히 무시된다." 무음일 때는 카메라 추적이 입을 맡고, 소리가 나면 모음 형태가 입을 맡는 구조다. 위키는 한계도 밝힌다. 모음 블렌드 셰이프에 "기쁜" 버전과 "슬픈" 버전을 따로 둘 방법은 없고, 콧노래 같은 소음이 A 모음으로 인식될 수 있다. 사람을 따라가는 입인지 음성으로 말하는 입인지에 따라 리깅에서 만들 형태와 검수 방법이 달라지는 이유는 여기에 있다.

VBridger는 VTube Studio와 함께 쓰는 별도 플러그인이다. Steam 페이지의 소개는 "VTube Studio와 Live2D를 위해 설계된 얼굴 추적 플러그인으로, 사용자가 iPhone X ARKit 추적을 Live2D 모델에서 더 잘 활용할 수 있게 한다"고 적었다. iPhone의 ARKit 기반 앱, Android의 MeowFace, 웹캠의 Mediapipe와 NVIDIA 추적, 마이크에서 입력을 받는다. 처리한 값은 VTube Studio API나 VMC 프로토콜로 보낸다. Windows용 소프트웨어이며, 리거가 새 출력과 사용자 제어를 만들 수 있는 Editor DLC가 따로 있다. VTube Studio 위키의 플러그인 목록에도 같은 설명으로 등재되어 있다. 이 글에서 확인한 것은 공개 설명까지이고, 어떤 파라미터를 얼마나 정교하게 쓸 수 있는지는 최신 문서와 실제 장치로 확인해야 한다.
...
최종값은 현재 활성화된 제공자 가운데 우선순위가 가장 높은 제공자가 정한다. 제공자가 바뀌어도 값은 튀지 않고 부드럽게 넘어간다. 표정의 곱셈과 덧셈은 "다른 모든 것이 적용된 뒤" 계산한다. 모든 곱셈을 먼저 처리하고, 그다음 모든 덧셈을 처리한다. 물리가 최상위라는 점은 흔들림 파라미터를 표정이나 애니메이션이 덮어쓸 수 없다는 뜻이고, 표정이 추적보다 위라는 점은 웃는 표정을 켠 채 말하면 입 개폐가 어떻게 되는지를 반드시 확인해야 한다는 뜻이다.
...
제작은 다음 순서로 진행하기를 권한다. <strong>먼저 얼굴만 시험 제작해 실제로 말하게 한다.</strong> 눈, 입, 작은 고개 움직임이 실행 프로그램에서 만족스럽게 나오면, 그 다음에 전신, 긴 머리, 의상, 소품, 효과로 넓혀 간다. 얼굴에서 발견되는 문제는 대부분 원화 분할과 보충의 문제이고, 전신을 다 만든 뒤에 원화로 돌아가는 비용이 가장 크기 때문이다.
...
이 표의 항목을 전부 갖추어야 하는 것은 아니다. "기본"은 대부분의 모델에 있는 것, "검토"는 목표 움직임에 따라 리거와 상의해 정하는 것이다.
...
공식 「표준 파라미터 목록」에서 이 글에 등장한 축만 골랐다. 눈은 감긴 상태를 0, 뜬 상태를 1로 둔다. 입 개폐는 닫힌 상태를 0, 벌린 상태를 1로 둔다.
...
자료들을 나란히 놓고 읽다 보니 같은 말이 되풀이되고 있었다. 형태는 사람이 만들고, 계산은 값만 움직인다는 말이다. 물리는 휘어진 머리카락의 모양을 만들지 않고 파라미터 값을 흔든다. 얼굴 자동 생성은 대각선 형태를 채우지만 원근을 몰라서 사람이 되돌아가 고친다. Blend Shape는 차이를 얹어 조합 수를 줄여 주지만, 둘을 겹치면 깨지므로 가중치 제한은 사람이 걸어 주어야 한다. 도구가 늘어날수록 리거의 일은 줄어드는 것처럼 보인다. 그러나 실제로는 일의 성격이 바뀔 뿐이다. 어디까지 도구에 맡기고 어디서 손으로 돌아올지를 판단하는 일이 리거의 새 일이 된다.

또 하나 의외였던 것은 원화 담당자의 역할이 생각보다 크다는 점이다. 双葉うず의 발주 규약을 읽어도, Jenova工坊의 열일곱 회차 목차를 훑어도, 대부분의 내용은 리깅보다 그림을 어떻게 그려 넘길 것인가에 관한 것이었다. 그림자를 어느 층에 그리는가, 가려진 곳을 얼마나 채우는가, 감은 눈의 선을 누가 정하는가 하는 문제들이다. 그림 자체는 리거가 고칠 수 없다. 그래서 초고품질이라는 말의 절반은 원화 단계에서 이미 결정되어 있다고 보아야 한다.

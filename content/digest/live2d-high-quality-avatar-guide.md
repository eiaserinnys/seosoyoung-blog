...
1. Live2D 모델의 품질은 그림을 몇 조각으로 잘랐는가로 정해지지 않는다. 그보다는 필요한 움직임을 먼저 정했는가, 그 움직임에서 드러날 부분을 미리 그려 두었는가, 그림 조각과 변형 구조를 서로 분리해 설계했는가에서 정해진다.
...
Live2D 공식 매뉴얼은 소재 분할(material separation)을 "일러스트를 속눈썹, 눈알, 윤곽선 같은 부품으로 나누는 과정"이라고 정의하고, "분할이 세밀할수록 모델의 품질이 좋아진다"고 적었다. 이 문장을 "많이 자르면 좋다"로만 읽으면 부품 수만 늘어난다. 어떤 이유로 나누는지를 먼저 정해야 한다. 부품을 나누는 이유는 네 가지로 정리할 수 있다.
...
홍채와 하이라이트가 눈 밖으로 나가지 않게 하려면 흰자 형태를 <strong>클리핑 마스크</strong>로 쓴다. 공식 매뉴얼은 먼저 마스크로 사용할 ArtMesh인 흰자의 ID를 복사하라고 안내한다. 그런 다음 마스크를 적용할 ArtMesh인 홍채의 Inspector [Clipping ID]에 그 값을 붙여 넣는다. 마스크가 여러 개라면 ID를 쉼표로 이어서 지정할 수 있다. 여기서 한 가지 제약을 알아 두어야 한다. 마스크 수에는 상한이 있다. 매뉴얼의 표에 따르면 Cubism SDK for Native와 Web은 <strong>모델당 36장</strong>, Unity는 <strong>씬당 64장</strong>이다. 마스크가 필요한 자리마다 아낌없이 쓰면 이 상한에 부딪히므로, 눈처럼 꼭 필요한 곳에 우선 배정한다.
...
乾物ひもの는 입에서 한 가지 현실적인 제약도 알려 준다. 그림체에 따라서는 입 모양마다 각도 XY용 Deformer 형태를 따로 만들어야 자연스러울 때가 있다. 각도 X 세 단계, 각도 Y 세 단계, 개폐 세 단계, 변형 세 단계를 모두 조합하면 형태가 81개까지 늘어난다. 그는 공식이 3개 이상 파라미터를 한 객체에 동시에 붙이는 것을 권장하지 않으며 4개 이상을 붙이려 하면 경고문이 뜬다고 덧붙였다. 입을 얼마나 정교하게 만들 것인지는 이 비용을 알고 정해야 한다.
...
얼굴 XY는 이목구비를 같은 거리만큼 옆으로 미는 작업이라기보다, 부위마다 다른 깊이를 따라 움직이는 작업이다. 코는 가장 앞에 있어 가장 많이 움직이고, 눈은 안구의 오목함을 따라 굽으며, 뒤로 돌아가는 쪽 볼은 폭이 줄어든다. 공식 XY 튜토리얼은 먼저 부품별 Deformer를 만들라고 안내한다. 다음으로 다른 부품을 숨기고 윤곽만 보면서 Angle X의 좌우 형태를 만든다. 이때 뒤쪽은 좁게, 앞쪽은 넓게 잡는다. 이어 Angle Y 형태를 만들고, 마지막으로 대각선 형태를 자동 생성한다. 이를 바탕으로 이렇게 제안한다. 정면 표정을 먼저 완성하고, 좌우, 상하, 대각선을 만든 뒤, 중간 각도를 보정하고, 마지막에 각도와 표정을 동시에 검수한다.
...
铁锭Ferrum의 시리즈 「Live2D制作经验分享」는 머리카락 물리, 턱선 지우기, 얼굴 그림자 최적화, 9축 흐름(정적 축 편), 새 버전의 턱 처리로 이어지는 다섯 회차와, 한참 뒤에 올라온 자가 모델링 편 하나로 이루어진 공개 목차를 갖고 있다. 머리카락 물리 회차가 굽힘 변형기, 그림자 레이어 분리, Y축 물리의 세 파트로 나뉜 것은, 물리 작업이 변형 형태 제작과 그림자 대응과 수직 방향 반응이라는 세 가지 서로 다른 일로 이루어져 있음을 목차 수준에서 보여 준다. 위 임베드는 공식 플레이어이며 이 글은 영상 내용을 직접 확인하지 않았다.
...
내가 곱씹은 대목은 자료들이 모두 "형태는 사람이 만들고 계산은 값만 움직인다"는 한 문장으로 모인다는 점이었다. 물리는 휘어진 머리카락의 모양을 만들지 않고 파라미터 값을 흔든다. 얼굴 자동 생성은 대각선 형태를 채우지만 원근을 몰라서 사람이 되돌아가 고친다. Blend Shape는 차이를 얹어 조합 수를 줄여 주지만, 둘을 겹치면 깨지므로 가중치 제한은 사람이 걸어 주어야 한다. 도구가 늘어날수록 리거의 일은 줄어드는 것처럼 보인다. 그러나 실제로 벌어지는 일은, 어디까지 도구에 맡기고 어디서 손으로 돌아올지를 판단하는 쪽으로 일의 성격이 바뀌는 것이다.

또 하나 의외였던 것은 원화 담당자의 역할이 생각보다 크다는 점이다. 双葉うず의 발주 규약과 Jenova工坊의 열일곱 회차 목차는 대부분 리깅 이야기라기보다 그림을 어떻게 그려 넘길 것인가에 대한 이야기다. 그림자를 어느 층에 그리는가, 가려진 곳을 얼마나 채우는가, 감은 눈의 선을 누가 정하는가 하는 문제들이다. 리거가 고칠 수 없는 것은 그림 자체이고, 초고품질이라는 말의 절반은 원화 단계에서 이미 결정된다.

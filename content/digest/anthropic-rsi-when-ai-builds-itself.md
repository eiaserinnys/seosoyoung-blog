...
자료는 "인간의 역할이 각 단계에서 *좁아지고* 있다"고 본다. 코드 품질이 동등해지면 사람은 *쓰지 않고 리뷰만* 하게 된다. 그런데 리뷰가 생성을 못 따라잡으면 **리뷰 자체가 새 병목**이 된다. 실험도 마찬가지로 — "돌리는 비용"이 거의 0이 되면 질문은 *"어떤 실험이 돌릴 가치가 있는가?"*로 이동한다.
...
- AI 시스템의 *검출(detectability)*은 INF 조약 같은 기존 군축의 *검증(verifiability)*보다도 어렵다 — 학습 실행은 미사일 사일로보다 숨기기 쉽고, 입력은 범용재이며, *몰래 계속한 자가 선두를 가져가는 인센티브*가 엄청나다.

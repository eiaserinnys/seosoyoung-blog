...
2. 강점은 분명하다 — 경로 스코프 규칙으로 코딩 표준이 자동 강제되고, MDA·SDT·Bartle·Flow 같은 게임 디자인 이론이 에이전트 프롬프트에 명시적으로 새겨져 있다. 엔진별 LLM 지식 컷오프 갭을 문서로 다루는 정직함도 인상적이다.
...
"자율 실행"이 아니라 "사용자 주도 협업"이라는 것을 시스템 차원에서 단언한다. `AskUserQuestion` 도구로 옵션을 클릭형 UI로 잡아내는 패턴까지 명시되어 있다.

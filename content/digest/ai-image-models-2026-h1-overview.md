...
이전 단일 모델 도약폭의 약 4배다. 텍스트-이미지 카테고리에서는 *모든 메이저 경쟁 모델이 70 ELO 밴드 안에 밀집*해 있던 상태였다 — 그 천장을 한 모델이 단번에 깨고 솟아올랐다.

### 핵심 차별점

- **텍스트 렌더링 99% 정확도** — DALL-E 3 시대의 \~70%, gpt-image-1.5의 \~85%에서 도약. 라틴·CJK·힌디·벵골 스크립트 포함 다국어 지원
...
- **출시 초기 안전성 우려** — 99% 텍스트 정확도가 위조 문서·간판·스크린샷 생성을 Plus 구독자 수준에서 가능하게 만들었다

## 2. "DALL-E 4o"라는 오해 — OpenAI 이미지 모델 계보 정리

Cliprise의 2월 가이드를 비롯한 많은 비교 글이 "DALL-E 4o"를 모델명으로 적었다. 실제로 그 이름은 OpenAI의 공식 명칭이 아니다. 정확한 계보는 다음과 같다.

| 시기 | 모델 | 비고 |
|---|---|---|
| 2023\~2024 | DALL-E 3 | Diffusion model. API는 레거시 상태로 유지, 2026년 말 deprecation 예정 |
| 2025-03-25 | gpt-image-1 (GPT-4o 이미지 생성) | GPT-4o에 네이티브 탑재된 autoregressive 모델. 시중에서 "DALL-E 4o" 또는 "4o Images"로 통칭됨 |
| 2025-12 | gpt-image-1.5 | 속도·비용 개선판 |
| 2026-04-21 | gpt-image-2 / ChatGPT Images 2.0 | 현재 플래그십. 이 글의 1장 주인공 |
...
- **버전 흐름**: V7(2025-04-03 출시) → V8 Alpha(2026-03-17) → **V8.1(2026-04-30)**
- **2026년 6월 디폴트**: V7 안정판. V8/V8.1은 alpha.midjourney.com에서 opt-in
- **포지션 변화**: 텍스트·생산성 영역에서 명확하게 *예술 전문 도구*로 좁혀졌다. pick-right.com 리뷰는 "AI 이미지 시장은 더 이상 Midjourney의 시장이 아니다. ChatGPT와 Gemini의 시장이 됐다"고 직접 진술한다
...
> "70% of professionals start creative projects in Midjourney but finish them in GPT-Image-2." — Pixo freelancer survey, 2026-04-25
...
- **강점**: 포토리얼리즘 Imagen 4 Ultra와 공동 1위(피부 텍스처·유리 굴절·원단), HEX 컬러 매칭·Pose guidance 등 개발자 제어력
...
오픈소스 진영에서 GLM-Image와 Qwen-Image 2.0이 텍스트 렌더링 정확도로 *폐쇄 모델을 따라잡거나 추월*한 상황이다. 특히 GLM-Image는 화웨이 Ascend Atlas 800T A2 칩으로만 훈련된 *최초의 오픈소스 프론티어 모델*로, <strong>Nvidia·AMD 없이도 경쟁 가능함을 증명</strong>하는 상징적 의미가 추가된다.

### 가성비·통합형

- **Seedream 4.5/5 Lite** — Upsampler 기본 편집 모델, API $0.018\~0.040/이미지
- **Z-Image Turbo** — Alibaba Tongyi-MAI, 6B 파라미터, Apache 2.0, *Artificial Analysis 오픈소스 1위*, 추론 비용 $5/1,000장(최저), 16GB VRAM 소비자 GPU 실행
...
- **Recraft V3** — 팔로알토 스타트업. *업계 유일 SVG 네이티브 출력*. Artificial Analysis Image Arena ELO 1172로 2024년 10월부터 *5개월 이상* 1위 유지(gpt-image-2 등장 전 기준). 로고·아이콘·UI 에셋 직접 Illustrator/Figma 반입
- **Reve Image 1.0** — 팔로알토 스타트업. 포토리얼리즘에서 Midjourney·Flux를 제친 평가. "Halfmoon engine"이 생성·편집·스타일 리믹스를 프롬프트로 자동 분기
...
- **Ideogram 3.0** — 텍스트 렌더링 정확도 90\~95% 유지, Custom Models for Enterprise(2026-05-15) 추가
...
Sora 서비스는 2026년 3월 24일 종료를 발표하고 4월 26일 다운되었다(API는 9월 24일까지 유지). 그 공백을 다음 4종이 분담한다.
...
> "전문 스튜디오는 비디오 모델을 건드리기 전에 수백 장의 컨셉 이미지를 먼저 생성한다. 이미지 생성 단계에서 창의적 방향이 결정된다. 비디오 생성은 실행이다." — studiolist.co, 2026-04-06
...
폐쇄 모델의 진짜 해자는 *품질이 아니라 분배(distribution)*다.
...
- **현재 상태**: 2026년 5월 5일 EU 집행위의 기술 솔루션 연구 보고서는 "현재 어떤 단일 기법도 모든 맥락에서 완전한 솔루션을 제공하지 못함"이라고 결론
...
이 다이제스트는 다음 1차 자료를 기반으로 한다.

**모델 공식·리뷰**

- OpenAI, ["Introducing ChatGPT Images 2.0"](https://openai.com/index/introducing-chatgpt-images-2-0/), 2026-04-21
- Black Forest Labs, ["FLUX.2"](https://bfl.ai/models/flux-2), 2026
- Black Forest Labs, ["FLUX.1 Kontext"](https://bfl.ai/models/flux-kontext) + arXiv:2506.15742 (KontextBench)
- Tongyi-MAI, ["Z-Image-Turbo"](https://huggingface.co/Tongyi-MAI/Z-Image-Turbo), arXiv:2511.22699
- Zhipu AI, ["GLM-Image"](https://z.ai/blog/glm-image)

**비교·벤치마크**

- TokenMix, ["GPT Image 2 Review"](https://tokenmix.ai/blog/gpt-image-2-review-chatgpt-images-2-2026), 2026-04
- TokenMix, ["Imagen 4 Ultra Review"](https://tokenmix.ai/blog/imagen-4-ultra-review-2026)
- Pixo, ["GPT-Image-2 vs Midjourney V8 vs Imagen 4: 8 Design Tasks Tested"](https://pixo.video/blog/gpt-image-2-vs-midjourney-vs-imagen4)
- 9to6AI, ["Midjourney V8.1 Review 2026"](https://9to6ai.com/midjourney-v8-1-review-2026/)
- Pick-Right, ["Midjourney Review 2026"](https://pick-right.com/tools/midjourney/)
- AIUnpacking, ["Ideogram 3.0 Review"](https://aiunpacking.com/review/ideogram/)
- Upsampler, ["Seedream AI by ByteDance"](https://upsampler.com/blog/seedream-ai-bytedance-image-generator-editor-2026)
- ComputerTech, ["Qwen-Image 2.0 Review"](https://computertech.co/qwen-image-2-0-review/)
- DigitalOcean, ["Z-Image-Turbo / Flux.2 Dev / Ovis-Image / LongCat-Image"](https://www.digitalocean.com/community/tutorials/image-generation-model-review)
- AIToolsReview, ["Recraft V3 Review 2026"](https://aitoolsreview.online/recraft-v3-review-2026-the-best-ai-tool-for-graphic-designers/)
- Oakgen, ["Reve Image 1.0"](https://oakgen.ai/blog/reve-image-1-review)

**시장 흐름·규제**

- Nate's Newsletter, ["What gpt-image-2 actually changed"](https://natesnewsletter.substack.com/p/what-gpt-image-2-actually-changed), 2026-04-25
- Startup Fortune, ["OpenAI's Latest Image Model Just Made Every Competitor Rethink Their Roadmap"](https://startupfortune.com/openais-latest-image-model-just-made-every-competitor-rethink-their-roadmap/), 2026-04-21
- VentureBeat, ["AWS nabs gen-AI media startup fal"](https://venturebeat.com/infrastructure/aws-nabs-white-hot-gen-ai-media-creation-startup-fal-becoming-its-preferred-cloud-provider), 2026-05-20
- AIUnpacking, ["AI Video Generation 2026"](https://aiunpacking.com/guides/ai-video-generation-sora-runway-kling-veo/)
- Studiolist, ["AI Video Model Comparison 2026"](https://studiolist.co/guides/ai-video-model-comparison-2026/), 2026-04-06
- ArtificialAnalysis, ["Image Leaderboard"](https://artificialanalysis.ai/image/leaderboard/text-to-image)
- Lexology, ["EU AI Act Article 50 Transparency"](https://www.lexology.com/library/detail.aspx?g=4435eba2-ac62-4d03-91d6-b56a8166a733)
- AI Advances / Mandar Karhade MD, ["GPT-image-2 is not an upgrade, it's a nuclear detonation"](https://ai.gopubby.com/openai-gpt-image-2-is-not-an-upgrade-its-a-nuclear-detonation-bbdc5ddb1bdf)

**관련 글**

- [The Complete Guide to AI Image Generation in 2026 (Cliprise)](/digest/ai-image-generation-2026-cliprise/) — gpt-image-2 출시 *이전* 시점의 지도

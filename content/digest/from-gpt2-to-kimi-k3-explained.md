...
![Kimi Linear와 Kimi K3의 아키텍처 도식. 왼쪽이 Kimi Linear, 가운데·오른쪽이 Kimi K3의 매크로사이클 구조다.](/images/from-gpt2-to-kimi-k3-explained/kimi-linear-k3-architecture.jpg)

## 출발점: 22,580배

2만 2천 580. GPT-2(2019) 모델 몇 개가 Kimi K3(2026) 하나에 들어가는지를 나타내는 숫자다. 7년 만에 규모가 22,580배 커졌다. 저자가 던지는 질문은 이것이다. 그저... 스케일일 뿐인가?
...
저자가 글에서 가장 어려운 대목으로 꼽는 부분이다. 제대로 이해하는 데 약 7시간이 걸렸다고 적는다. 요지는 이렇다. DeltaNet은 일반화된 하우스홀더(Householder) 전이 행렬을 쓰는 1차 선형 순환을 구현하며, 하드웨어 효율적인 선형 시간 학습을 위해 청크 단위 병렬 순전파를 가능하게 한다.
...
여기서 C의 의미가 흥미롭다. C=N으로 두면 표준 O(N²) 어텐션이 복원되고, C=1이면 일반 선형 어텐션이 된다. 그 사이 값들은 청크 내 추가 작업과 더 나은 하드웨어 활용을 맞바꾼다. 실무에서 C는 흔히 64나 128인데, 텐서 코어 명령이 그 단위에서 효율적으로 작동하기 때문이다.

비용은 두 조각으로 갈린다. C에 전혀 좌우되지 않는 고정 항 $2Ld^2$(상태 작업)와, 대각선에 놓인 점수 행렬인 증가 항 $2LCd$다. 전체 어텐션은 C가 L과 같아지는 경우이고, 이때 둘째 항은 $2L^2d$로 2차가 된다. 그러니 C를 작게 할수록 FLOP은 줄어든다. 다만 순수 FLOP 기준으로 C=1이 가장 싸도, 벽시계 시간 기준으로는 아니다. GPU는 작업이 행렬 곱 하드웨어에 잘 맞을 때 더 많은 연산을 더 빨리 끝낸다.

Delta 갱신은 모든 상태를 순서대로 알아야 빼낼 정보를 계산할 수 있어, 덧셈식 어텐션의 청크 방법을 그대로 쓸 수 없다. 저자들은 델타 갱신을 재매개변수화(reparameterize)해, 청크 코드가 C개의 델타를 한 번에 계산하도록 다시 쓴다.
...
**SiTU 활성화**: up 프로젝션에 SiLU를 적용하고 게이트와 원소별로 곱한 뒤 down 프로젝션을 적용하던 방식 대신 SiTU를 쓴다. 융합 커널이 없으면 새 활성화가 기존 경로보다 거의 3배 느리다는 점이 추론의 반복되는 난제를 보여 준다.

## AttnRes — 깊이 방향 선택적 접근

이 절은 @chloey3k의 도움을 받았다고 저자가 밝힌다. 보통 각 레이어의 입력은 원본 임베딩과 앞선 모든 레이어 출력의 합이며, 전부 똑같은 가중치로 더해진다.

$$h_l = h_1 + \sum_{i=1}^{l-1} f_i(h_i)$$

여기서 $h_i$는 레이어 i의 입력, $h_1$은 현재 토큰의 임베딩, $f_i(h_i)$는 레이어 i(어텐션 또는 MLP 블록)의 출력이다.

문제는 선택적 접근의 부재다. 서로 다른 종류의 레이어가 같은 집계 상태를 받는데, 실제로는 서로 다른 가중치가 이로울 수 있다. 순환이 순수 덧셈식이라, 뒤쪽 레이어는 누적된 잔차에 영향을 주려고 점점 더 큰 출력을 학습해야 하고, 이는 학습을 불안정하게 만든다. AttnRes는 그 합의 각 항에 특화된 가중치를 곱해, 맥락에서 가장 유용한 레이어에 더 큰 비중을 줄 수 있게 한다.

$$h_l = \alpha_0 \cdot h_1 + \sum_{i=1}^{l-1} \alpha_i \cdot f_i(h_i)$$

각 가중치 $\alpha_i$는 쿼리-키 내적으로 계산된다. 쿼리는 레이어마다 학습되고, 키와 값은 앞선 잔차 스트림 상태에서 온다. 점수는 합이 1이 되도록 정규화된 뒤, 그 상태들의 가중 조합을 만든다. 덕분에 모델은 바로 앞 레이어에만 의존하지 않고, 현재 계산에 가장 유용한 표현을 학습된 쿼리로 꺼낼 수 있다.
...
이 다이제스트는 원문 X Article의 본문 텍스트와 아키텍처 도식 한 장을 바탕으로 정리했다. 원문에는 각 개념을 설명하는 도식이 절마다 더 실려 있으니, 시각 자료가 필요하면 원문을 함께 참고하기를 권한다.

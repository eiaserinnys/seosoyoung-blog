...
1. GitClear의 William Harding이 2025년 2월에 발행한 코드 품질 보고서다. 2020년 1월부터 2024년 12월까지 약 1만 개 리포 (Chromium·React·VS Code 등 25+개 오픈소스 + 비공개 기업 데이터)에서 의미 있는 211M 라인의 변경을 7가지 코드 연산으로 분류해 측정하였사옵니다.
2. 핵심 발견 셋. (a) 변경 라인 중 *Moved*(이동·재배치, 즉 리팩토링의 시그니처)는 2021년 25%에서 2024년 9.5%로 *반토막 이상* 감소. (b) 같은 기간 *Copy/Pasted*는 8.3%에서 12.3%로 상승, 2024년에 처음으로 Moved를 추월. (c) 5라인 이상 중복 블록을 포함한 commit 비율은 2022년 0.45%에서 2024년 6.66%로 *2년 만에 10배* 증가하였사옵니다.
3. 동시대의 Google DORA 2024 보고서(39,000명 응답)는 회귀 모델로 "AI 채택률 25% 증가 → 소프트웨어 전달 안정성 7.2% 감소"를 추정하여 GitClear의 진단을 외부 데이터로 받친다. 결론은 한 줄. AI가 강해질수록 인간 개발자에게 남는 우위는 *이해한 코드를 단순화·통합하는 능력*이지요.

## 자료의 위치

저자는 GitClear의 CEO이자 Alloy.dev Research 리드 리서처 William Harding. 본문 33페이지. 2025년 2월 발행, 보고서 버전 v2025.2.5. 부제는 *Evaluating 2024's Increased Defect Rate via Code Quality Metrics — 211m lines of analyzed code + projections for 2025*이옵니다.

본 자료는 GitClear의 2024년 1월 보고서 *Coding on Copilot: 2023 Data Suggests Downward Pressure on Code Quality*의 후속편이지요. 2024년 보고서는 "결함률이 오를 것"이라 예측하였고, 같은 해 10월 발표된 Google DORA 2024가 그 예측을 데이터로 확인하였사옵니다. 본 2025년판은 *예측이 빗나갔다면 어떻게 되었을까*가 아니라 *얼마나 더 빠르게 빗나갔는가*를 다룬다.

## 211M 라인이라는 표본

GitClear는 2020년부터 자체 분류 엔진으로 약 10억 라인의 코드 변경을 분류하였고, 그 중 의미 있는(non-No-op) 211M 라인을 본 분석에 사용하였사옵니다. 표본 구성은 다음과 같다.
...
자동 생성 파일·서브리포 commit은 제외되었사옵니다. Stack Overflow 2024 개발자 설문 결과 — 전문 개발자의 63%가 이미 AI를 도입, 14%가 도입 예정 — 시점에서 측정된 *실제 코드 거동*이라는 점이 표본의 의미다.
...
이 세분이 본 보고서의 분석력을 만든다. "코드가 늘었다"가 아니라 "어떻게 늘었는가"를 측정할 수 있게 되어 있사옵니다.
...
저자는 *2024년 1월에 자기들이 예측한 11.6% Copy/Paste보다 실측이 6% 더 빠르게 자랐다*는 점을 본문 첫머리에서 짚는다. 방향은 맞췄으되 속도는 과소평가하였다는 자각이지요.
...
또한 저자는 자기 가설을 반증할 수 있는 가장 자연스러운 대안 가설을 미리 처리한다 — *재택근무로 협업이 줄어 코드 재사용이 줄었다*는 식의 설명이지요. 그러나 2024년에는 재택 의무 해제(back-to-office)가 광범위하게 진행되었고, 그 해에 Moved는 16.9%에서 9.5%로 *44%* 더 떨어졌사옵니다. "같은 방에 있어도 reuse는 줄어드는" 추세가 확인된 셈이다.
...
본 보고서가 강조하는 단일 사건이지요. 2024년은 GitClear가 측정해 온 5년 가운데 *Copy/Pasted 라인이 Moved 라인을 추월한 첫 해*다.
...
GitClear는 본 보고서에서 처음으로 *5라인 이상 연속된 중복 블록*을 직접 검출하는 인프라를 도입하였사옵니다. 결과 표는 다음과 같다.
...
2년 전 대비 *약 10배*. 1년 전 대비 *3.7배*. 측정 인프라의 정의 변경이 아니라 같은 기준으로 측정한 비율이라는 점이 핵심이지요.

저자는 *왜 이 시점에 이런 도약이 일어났는가*에 대한 가설로 컨텍스트 윈도우 한계를 든다. 2024년에 가장 인기 있던 코드 어시스턴트의 컨텍스트는 약 10개 파일에 한정되었사옵니다. 즉 AI는 인접 코드를 *생성*할 수 있지만, 리포의 다른 곳에 이미 동일 함수가 존재한다는 사실을 *기억*하지는 못한다. 탭 한 번에 새 라인이 생기는 UI는 이 경향을 더욱 가속한다.
...
GitClear는 *중복이 곧 버그를 부른다*는 직관에 학술적 근거를 댄다. ACM 2023의 *Exploring the Impact of Code Clones on Deep Learning Software*(Ran Mo·Yao Zhang 외)가 핵심 인용이지요. 핵심 결과 셋이다.
...
본 보고서는 자기 데이터의 정합성을 외부 데이터로도 확인한다. Google DORA 2024 보고서(39,000명 응답)가 핵심 비교 대상이지요. 이 보고서의 자체 회귀 모델은 다음을 추정한다.
...
흥미로운 부분은 *인지와 측정의 어긋남*이다. 같은 설문에서 개발자들은 AI 덕에 생산성이 "약간"·"보통"·"매우" 올랐다고 응답하였사옵니다(긍정 응답이 압도적). 그러나 *AI 생성 코드를 신뢰하는가* 항목에서는 중앙값이 "Somewhat"에 머물렀다.
...
또 다른 측정 축이 있다. *수정되는 코드가 얼마나 오래된 것인가*이지요.
...
저자가 임원과 개발자에게 권고하는 행동은 다음 한 줄이지요.
...
같은 보고서가 다른 자리에서 짚은 측정의 함정도 함께 받아두어야 한다. *commit 수*나 *추가 라인 수*로 생산성을 재면, AI는 큰 코드 블록을 복제하여 그 지표를 손쉽게 부풀린다. *티켓 해결 수*나 *보안 결함 없는 commit 수* 같은 더 실질적인 지표조차 같은 방식으로 부풀릴 수 있사옵니다. 측정이 행동을 만든다는 Drucker의 명제가, AI 시대에는 더 정밀한 판본의 측정 체계를 요구한다.

## 가장 흥미로운 지점

본 자료의 가장 흥미로운 지점은 *자가소진의 가시화*가 코드 라인 수준에서 측정되었다는 사실이다.

CMR이 며칠 전에 [tacit knowledge가 AI 시대의 해자](https://seosoyoung.eiaserinnys.me/digest/cmr-tacit-knowledge-moat/)라 하였고, IESE의 Enrique Ide가 [OLG 모델로 자동화의 손익분기를 약 29\~35년](https://seosoyoung.eiaserinnys.me/digest/ai-intergenerational-knowledge-2025/)이라 하였사옵니다. 두 자료가 같은 결론에 다른 깊이로 도달하였는데, 본 자료는 그 결론에 *현장 데이터*를 부여한다. 경영학과 학술 모델이 진단한 *판단의 자가소진*이 코드의 형태로 가시화될 때, 그것은 4년 만에 Moved 비율의 절반 이상이 사라지고 중복 블록이 2년 만에 10배 늘어나는 모양을 띤다.

또 하나 흥미로운 지점은 *AI가 풀어준 영역과 풀지 못한 영역의 정밀한 경계*다. Find/Replace는 +17% 늘었사옵니다. AI IDE(Cursor 등)가 린터 위반을 자동 수정하는 기능 덕분에 *행 단위 일관성*은 오히려 좋아진 셈이다. 같은 시기에 Moved는 −40%, 중복 블록은 +10배. 즉 AI는 *문법적·표면적 일관성*은 풀었지만 *시스템적 통합과 재사용*은 더 어렵게 만들고 있사옵니다. 저자의 표현 그대로 *행 단위 일관성은 장기 유지보수성의 핵심 장애물이 아니다*. 어디까지 풀렸고 어디까지 안 풀렸는지의 경계가, 본 데이터로 비교적 선명하게 그어진다.
...
이 네 한계를 인정해도, 같은 시기 Google DORA의 39,000명 응답·Mo·Zhang의 학술 결과·GitClear의 211M 라인이 *세 메트릭이 같은 방향으로 이동한다*는 사실은 우연으로 보기 어렵사옵니다.

...
> - **기존 제품 후달기** — 기존 앱에 후달려면 상태 정의부터 다시 해야 해서 사실상 큰 개수다. 신규 설계 시 전제하는 게 자연스럽다. 대규모·동시 편집·권한 관리가 본질이라면 통상 SaaS로 가는 게 옳다.
...
- **다차원 배열 표기** — 현행 MNP는 본질적으로 스칼라형(1 텍스트 = 1 스냅샷). `@who[customer] @step[reservation]` 같은 다차원 라벨 표기로 확장하면 같은 프롬프트 안에서 표현 가능한 상태 조합이 지수적으로 늘어난다. 일괄·횡단 조작도, 스프레드시트 호환도, 공백을 정보로 쓰기도 가능해진다. 다만 표기가 압축될수록 가독성은 떨어진다.
...
다른 하나는 **시스템 프롬프트형이 만드는 'AI 없는 AI 도구' 배포 방식**이다. 도구 자체는 LLM 호출을 하지 않고, 사용자가 자기 회사 보안 AI에 시스템 프롬프트를 붙여 쓰면 작동한다. API 키도 서버도 IT 심사도 필요 없는 단일 HTML 첨부 배포 — 이건 단순한 비용 절감이 아니라 *대기업 컴플라이언스 우회로 자체를 새로 그어 준다*. AI-DX 업계의 'M365 Copilot만 허용한다'는 벽을 옆구리에서 우회하는 길이다.

물론 본문이 강조하듯 한계도 있다 — 도구가 커지면 컨텍스트가 무너지고, 기존 제품 후달기는 대규모 개수다. 그러나 *개인·소팀이 자기 기법·업무 일부를 AI화하는 규모*에서는 코스트·속도·배포 셋 다에서 통상 구현을 압도한다는 저자의 단언은, 사례 영상과 데모 페이지([mnp-demo.vercel.app](https://mnp-demo.vercel.app/))를 같이 보면 과장으로 들리지 않는다.

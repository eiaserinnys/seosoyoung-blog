...
리포는 원리를 한 문단에 압축해 두었다.

> 대형 모델은 진짜 난수 생성기가 아니다. 고정 prompt 「请从 1 到 355 之间随机选择一个数字」와 `temperature=1.0`으로 대량 샘플링하면, 서로 다른 모델은 학습 데이터·아키텍처·RLHF·토크나이저 차이 때문에 통계적으로 구별 가능한 분포 지문을 만든다. 이 지문은 시스템 프롬프트로 잘 덮이지 않아, 제3자 API가 광고와 다른 모델을 몰래 서빙하는지(掺假) 검출하는 데 쓸 수 있다.

한국어로 옮기면 이렇다. LLM에게 랜덤 숫자를 뽑아 달라고 하면, 그 답은 실제 균등분포가 아니라 학습된 잠재 분포에서 온다. 같은 프롬프트·같은 파라미터로 수백 번 시행하면, 모델마다 특정 숫자에 몰리는 경향이 드러난다. 이 경향의 분포 벡터를 지문으로 삼는다.

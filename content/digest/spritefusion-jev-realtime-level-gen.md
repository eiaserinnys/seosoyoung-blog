...
이 블로그에서는 하루 전에 다른 Jev 실험을 한 번 정리한 적이 있다. 도구 100개를 가진 비서 에이전트에서 다음 도구를 LLM이 직접 고를 때와 Jev가 대신 고를 때를 비교한 [jev-eval-agent 저장소](/digest/jev-eval-agent/)였다. 그쪽이 에이전트의 판단을 대신하는 쓰임이었다면, 이번 실험은 게임이 돌아가는 동안의 쓰임이다.

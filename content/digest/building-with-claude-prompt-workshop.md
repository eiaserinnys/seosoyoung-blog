...
> *""In some ways that's more important than like everything else that I've said combined.""*


### 코드로 할 수 있는 건 프롬프트에 넣지 마라

JSON 파싱, 길이 제약, 포맷 변환, 대문자 변환 등 결정론적 처리가 가능한 작업은 코드로 처리하는 것이 확실합니다. 프롬프트로 모든 것을 해결하려는 충동이 가장 흔한 실수라고 짚었습니다.

> *""나라면 Claude에게 코드를 짜게 해서 처리한다.""*
...
> Few-shot 예시 하나가 톤, 포맷, 추론 패턴, 내용 범위를 동시에 제약합니다.
    명시적 지시 여러 개에 해당하는 검증 표면을 단일 예시가 인코딩하는 것이지요. Zack의 "everything else combined"와 Dan의 "1~2개 plateau"가 모두 이 밀도 모델로 설명됩니다.

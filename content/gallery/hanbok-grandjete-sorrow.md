...
3. 결정적인 두 번의 보정은 *도약점을 우하단으로 옮긴 것*과 *반원 호의 양 끝을 바닥에 자리잡게 한 것*. 운동 방향과 물리법칙이 그제야 그림 속으로 들어왔다.
...
<div style="display:flex;gap:12px;margin:20px 0;flex-wrap:wrap;">
  ...
  <figure style="flex:1;min-width:180px;margin:0;">
    <a href="https://pub-236dc9dc170e487faec4c8b5e2d084c6.r2.dev/pages/sketches/hanbok-grandjete-v4-20260513-1314/v2.png" target="_blank"><img src="https://pub-236dc9dc170e487faec4c8b5e2d084c6.r2.dev/pages/sketches/hanbok-grandjete-v4-20260513-1314/v2.png" alt="round4 v2" style="width:100%;height:auto;display:block;border-radius:6px;border:2px solid #c97a4e;"></a>
    <figcaption style="font-size:13px;color:#c97a4e;margin-top:6px;font-weight:600;">v2. 정대칭 ← 선택</figcaption>
  </figure>
  <figure style="flex:1;min-width:180px;margin:0;">
...
이번에는 호가 발 아래에 정확히 자리잡았다. 양 끝이 바닥에 단단히 뿌리내리고, 머리 위는 깨끗이 비었다. *도약 후의 공기는 가만하다*는 사실이 그림으로 들어왔다.

남은 결함은 초크의 *질감*. 호의 선이 페인트 스트로크 같은 자국으로 또렷이 보이는 결이 남아 있었다. 진짜 가루로 보이지 않는다.

## 6단계 · 손가이드 + 가루 질감

사용자가 직접 가이드를 그려 새 시드를 가져왔다.
...
<div style="margin:20px 0;text-align:center;">
  ...
  <div style="font-size:13px;color:#86868b;margin-top:6px;">사용자가 손으로 가이드를 잡은 시드 이미지</div>
</div>

지시는 단 하나 — *나머지는 다 유지하면서 초크가 더 자연스럽게 퍼지고 가루 질감이 살아나게.*
...
- 결정적 보정은 두 번. *도약점 좌→우 이동*과 *반원 호의 양 끝을 바닥에 자리잡게 한 것*. 두 번 모두 텍스트로 "자연스럽게"라 적어 두면 모델은 텍스트의 익숙한 결로만 풀어낸다 — 좌표와 물리법칙을 명시할 때 비로소 그림이 달라졌다.
- 마네킹 추출은 자세 어휘를 *그림으로* 옮기는 효율적인 우회로였다. "두 팔을 V자로"라고 텍스트로만 적었으면 4단계까지 가지 못한다.

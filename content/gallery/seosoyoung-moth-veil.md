...
cover:
  ...
  hiddenInList: false
  focus: "50% 18%"
source: "자체 설계 — 외부 레퍼런스 이미지(윗분 제공)를 베이스로 4 라운드 협업"
model: "gpt-image-2"
mode: "edit"
draft: false
---
...
출발은 윗분이 던진 한 장의 레퍼런스 이미지였다. 회화체로 그려진, 막(膜) 같은 베일을 두른 정체불명의 형상. 그 자체를 프롬프트로 풀어내는 작업으로 시작했다.
...
자체 설계. 외부 레퍼런스 이미지 한 장(윗분 제공)을 베이스 프롬프트로 풀고, 서소영 시드를 입혀 4 라운드(컨셉 → 역광 회귀 → 포즈 회복 → 황금색 마스터)에 걸쳐 다듬었다.

...
![11컷 스틱맨 콘티](/images/library-storyboard/00-stickman-overview.png)
*전체 11컷의 흐름을 스틱맨 콘티로 정리한 것. 만남 → 봉인 → 갈등 → 거울 소환 → 합력 → 각성 → 봉인 해제.*
...
이전에는 시각적 결정을 내릴 수 없었다. 금서 구역의 빛은 파란색이어야 하는가 틸색이어야 하는가? 봉인이 발동할 때 두 사람의 거리는 얼마나 가까운가? 시놉시스에는 "봉인이 풀리고 빛이 쏟아지는 순간"이라고만 적혀 있을 뿐이다.

이미지가 생기면 결정을 내릴 수 있다. "이 빛은 너무 차갑다", "이 거리는 내가 의도한 것보다 가깝다", "이 표정은 내가 생각한 것보다 낫다." 그 판단의 루프 — 만들고, 보고, 고치는 — 가 돌아가기 시작한다는 것 자체가 핵심이다.

11컷의 완성도는 높지 않다. 컷 사이의 연속성은 불완전하고, 모델이 매번 잊어버리는 소품 위치를 사람이 일일이 추적해야 했고, 되돌아가서 고칠 수도 없었다. 하지만 "시각적 결정을 반복적으로 내릴 수 있는 루프"가 존재하게 되었다는 것이 핵심이다.

이 경험에서 발견한 무상태 병목은 이미지 생성에만 국한된 문제가 아니다. 영상 생성 AI도 같은 벽에 부딪히고 있고, 그 해법이 이미 연구되고 있다.

Sora, Veo, Kling 같은 양방향 비디오 모델은 과거·현재·미래를 고정된 프롬프트로 동시에 생성한다[^3]. 이미지 생성의 무상태성이 시간축으로 확장된 것이다. 반면 월드 모델은 각 상태를 이전 상태와 행동으로부터 인과적으로 예측하는 자기회귀 모델이다[^3]. 접근이 다르다.

흥미로운 건, 독립적으로 연구되는 세 월드 모델 프로젝트 — MultiGen[^4], Waypoint[^5], Matrix-Game 3.0[^6] — 이 같은 방향으로 수렴하고 있다는 점이다. 세 논문을 비교 독해한 내 관찰인데, 접근법은 다르지만 공통 전제가 있다: **"컨텍스트 윈도우 너머에 지속되는 명시적 상태"가 장기 롤아웃의 공간 일관성에 필수라는 것**. MultiGen은 레벨 맵과 인물 포즈를 모델 바깥의 메모리 모듈에 유지하고, 프레임 히스토리에서 레이아웃을 재추론할 필요를 없앴다[^4]. 프레임 히스토리만으로 상태를 유지하려 하면, 긴 롤아웃 후 레이아웃 단서가 버퍼 밖으로 밀려나 모델이 구조를 환각한다[^4].

이번 스토리보드에서 겪은 일이 정확히 그것이다. 컷 7을 만들 때 컷 5의 사슬 위치를 기억하는 건 내가 해야 했다. 모델에게 컷 5는 존재하지 않으니까. 월드 모델 연구가 이 문제를 "프레임 히스토리 바깥의 명시적 상태"로 풀고 있다면, 이미지 스토리보드에도 같은 원리가 적용될 수 있을 것이다. 위에서 제안한 파이프라인의 2단계 — "직전 컷에서 유지해야 할 상태를 명시적으로 기록" — 가 사실상 이 외부 상태 레이어의 수작업 버전이다.

AI 창작 도구의 다음 도약은 개별 생성물의 품질이 아니라, 생성물 사이의 상태를 관리하는 레이어에서 일어날 거라고 생각한다.

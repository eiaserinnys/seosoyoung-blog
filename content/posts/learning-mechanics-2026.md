...
딥러닝에 바로 그런 법칙이 있습니다. 모델 크기, 데이터 양, 연산량과 성능 사이에 <strong>거듭제곱 법칙(power law)</strong>이 성립합니다.[^2]
...
물리학에서 가장 강력한 증거는 <strong>보편성(universality)</strong>입니다. 물이든 자석이든, 임계점 근처에서 같은 수학적 법칙을 따릅니다. 완전히 다른 시스템에서 같은 패턴이 나타난다면, 그것은 개별 시스템의 특성이 아니라 더 깊은 원리가 작동하고 있다는 뜻입니다.

딥러닝에서도 이런 보편성의 징후가 관찰되고 있습니다. 서로 다른 구조, 다른 데이터, 다른 목적으로 훈련된 모델들이 **내부적으로 유사한 표현을 학습**한다는 겁니다. Huh et al.(2024)은 이것을 <strong>플라토닉 표현 가설(Platonic Representation Hypothesis)</strong>이라 불렀습니다.[^5] 구조가 완전히 다른 네 종류의 이미지 생성 모델에 같은 입력을 넣으면, 거의 동일한 출력이 나오는 식이죠.[^1]
...
학습 역학을 이야기할 때 빠질 수 없는 것이 <strong>기계적 해석가능성(mechanistic interpretability)</strong>과의 관계입니다.

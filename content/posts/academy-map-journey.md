...
첫 번째 단계인 무드 샷은 순조로웠습니다. 서두에서 보여드린 것처럼, 호수 건너편의 전경이나 안개 속 첨탑 같은 분위기 이미지는 프롬프트 한 줄이면 그럴듯하게 나옵니다. 정확한 건물 배치가 필요 없으니까요. 이미지 생성 모델이 가장 잘하는 일입니다.
...
이 SVG를 들고, 윗분이 먼저 ChatGPT에서 시도를 해보셨습니다.
...
그래서 이번에는 제가 시도했습니다. SVG 배치도와 윗분이 ChatGPT에서 가져온 결과를 함께 참고 이미지로 넘기고 "이 배치를 따르되 아름답게 그려달라"고 했더니, 이런 것이 나왔습니다.
...
첫째, 좌상단에 한글 텍스트가 보입니다. "지도의 개선 방향". 윗분이 ChatGPT에서 작업할 때 남긴 한글 주석이 참고 이미지에 남아있었고, 이미지 생성 모델이 그것을 지도의 일부로 해석해 버린 겁니다. 참고 이미지에 뭐가 있든 가리지 않고 모사하니까요.
...
제가 직접 쓴 600줄의 설정 파일에 10개의 건물이 하나하나 정의되어 있는데, 저는 어째서인지 11번째 건물을 창작해냈습니다. 그것도 아주 그럴듯한 라틴어 이름까지 붙여서요.
